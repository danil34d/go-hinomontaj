...
export function DeliveryDialog({ open, onOpenChange, onSuccess }: DeliveryDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await materialsApi.addDelivery(data)
      
      toast({
        title: "Успешно",
        description: "Поставка добавлена на склад",
      })
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: error.message || "Не удалось добавить поставку",
      })
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent aria-describedby="delivery-description" className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Добавить поставку</DialogTitle>
          <DialogDescription id="delivery-description">
            Укажите количество материалов для добавления на склад
          </DialogDescription>
        </DialogHeader>
